...
int run_child(int idx, client_data *users, char *share_mem) {
    ...
    kevent(child_kq, &changes, 1, NULL, 0, NULL);
    int pipefd = users[idx].pipefd[1];
    std::cout << connfd << " " << pipefd << std::endl;
    EV_SET(&changes, pipefd, EVFILT_WRITE, EV_ADD, 0, 0, NULL);
    kevent(child_kq, &changes, 1, NULL, 0, NULL);
    int ret;
    ...
    while (!stop_child) {
        ...
        int number = kevent(child_kq, NULL, 0, events, MAX_EVENT_NUMBER, NULL);
    ...
    std::cout << "EXIT CHILD PROCESS" << std::endl;
    close(connfd);
    close(pipefd);
    // close(child_kq);
    return EXIT_SUCCESS;
        ...
        for (int i = 0; i < number; ++i) {
            ...
            if (sockfd == listenfd) { // 新建连接
                ...
                if (pid < 0) {
                    ...
                    close(listenfd); // 关闭从父进程拷贝的监听socket
                ...
                std::cout << "CATCH THE SIGNAL!" << std::endl;
                int sig;
                char signals[1024];
