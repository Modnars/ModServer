...
int run_child(int idx, client_data *users, char *share_mem) {
    ...
    // 子进程使用IO复用技术来同时监听两个文件描述符: 客户连接socket、与父进程通信的
    // 文件描述符。
    ...
    while (!stop_child) {
        ...
        for (int i = 0; i != number; ++i) {
            ...
            if ((sockfd == connfd) && (events[i].events & EPOLLIN)) {
                ...
                // idx*BUFFER_SIZE处，长度为BUFFER_SIZE字节。因此，各个客户连接的读缓
                // 存是共享的
        ...
        for (int i = 0; i != number; ++i) {
            ...
            if (sockfd == listenfd) {
                ...
                if (pid < 0) {
                    ...
                    // 记录新的客户连接在数组users中的索引值，建立进程pid和该索引值
                    // 之间的映射关系
